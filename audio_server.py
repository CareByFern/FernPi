...
def grab_audio(shared_audio, shared_time, shared_pos, lock, n=None):
    ...
    print("grabbed some audio")
    print audio_signal
    return audio_signal[n:], time_stamps[n:]

def get_baseline(shared_audio, shared_time, shared_pos, lock):
    (signal, time) = grab_audio(shared_audio, shared_time, shared_pos, lock, 5)
    baseline = np.mean(signal)
    print "found baseline = "
    print baseline
    return baseline

def get_dbs(signal, baseline):
    dbs = 20*np.log10(signal/baseline)
    return dbs

def process_requests(shared_audio, shared_time, shared_pos, lock, baseline=0):
    """
    ...
    while True:
        ...
        time_stamps = np.roll(time_stamps, shift=buffer_len-current_pos)
        audio_signal = np.roll(audio_signal, shift=buffer_len-current_pos)
        dbs = get_dbs(audio_signal, baseline)
        #print "last five dbs and timestamps"
        real_timestamps = []
        for stamp in time_stamps[-5:]:
            real_timestamps.append(datetime.fromtimestamp(stamp).strftime('%Y-%m-%d %H:%M:%S'))
        #print real_timestamps
        #print dbs[-5:]
        # return results to webserver
        results = {'timestamps': real_timestamps,
                'dbs':dbs}
        conn.send(results)
        conn.close()
...
def init_server():
    ...
    p1.start()
    sleep(3)
    baseline=get_baseline(shared_audio, shared_time, shared_pos, lock)
    print "FINISHED INITIALIZING"
    p2 = mp.Process(target=process_requests, args=(shared_audio, shared_time, shared_pos, lock, baseline))
    p2.start()
